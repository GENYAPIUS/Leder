...
namespace Leder.Models.Cart
{
    ...
    public static class Operation
    {
        ...
        public static Cart GetCurrentCart()
        {
            ...
            if (HttpContext.Current.User.Identity.IsAuthenticated && HttpContext.Current.Session["CartId"].ToString() != HttpContext.Current.User.Identity.Name)
            {   //有登入  而且現在Id != UserName  的話ID套用UserName
                ...
                if(((Cart)cache[HttpContext.Current.Session["Non-membersId"].ToString()]).Count!=0)
                {
                    shift = true;   //設定為需要轉移
                }
            }
